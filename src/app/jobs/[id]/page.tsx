...
export default function JobDetailPage({ params }: JobDetailPageProps) {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Check bookmark
        const bookmarked = data.bookmarks.some((b: any) => b.job.id === id);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Back button */}
      <Link href="/" className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white mb-6 group transition-colors">
              ...
              <div className="space-y-4">
                {relatedJobs.map((rJob) => (
                  <div key={rJob.id} className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700/80 rounded-xl p-4 shadow-sm hover-card">
                    ...
                    <Link href={`/jobs/${rJob.id}`} className="block mt-1">
                    ...
                    <div className="flex justify-between items-center mt-3 pt-2 border-t border-slate-50 dark:border-slate-700/50 text-slate-400 text-xs">
                      ...
                      <Link href={`/jobs/${rJob.id}`} className="font-semibold text-indigo-600 dark:text-indigo-400">View Details</Link>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showApplyModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center">
            <div className="fixed inset-0 bg-slate-955/60 backdrop-blur-sm transition-opacity" onClick={() => setShowApplyModal(false)}></div>
            ...
            <div className="inline-block w-full max-w-lg p-6 my-8 overflow-hidden text-left align-middle bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 transition-all transform relative">
              ...
              <form onSubmit={handleApplySubmit} className="space-y-4">
                <div>
                  <label htmlFor="modalName" className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">Full Name</label>
                  <input
                    type="text"
                    id="modalName"
                    required
                ...
                <div>
                  <label htmlFor="modalEmail" className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">Email Address</label>
                  <input
                    type="email"
                    id="modalEmail"
                    required
                ...
                <div>
                  <span className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">Resume Document</span>
                ...
                <div>
                  <label htmlFor="coverLetter" className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">Cover Letter (Optional)</label>
                  <textarea
                    id="coverLetter"
                    rows={4}

...
const Home = () => {
  ...
  const [seekerBookmarks, setSeekerBookmarks] = useState<string[]>([]);
  ...
  const fetchJobs = useCallback(async () => {
    try {
      ...
      queryParams.append('page', activeFilters.page.toString());
  ...
  const fetchBookmarks = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setSeekerBookmarks(data.bookmarks.map((b: any) => b.job.id));
      }
    } catch (err) {
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex">
            <div className="w-screen max-w-xs bg-white dark:bg-slate-800 flex flex-col p-6 shadow-xl relative animate-in slide-in-from-right duration-250">
              <button
                ...
                className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-650"
              >
                <X className="w-5 h-5" />
              </button>
